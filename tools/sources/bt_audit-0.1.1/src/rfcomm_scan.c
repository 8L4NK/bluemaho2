...
/* some code improvements (timeout, hci_dev spec, accuracy) by ^_^ 2009 */
/* thanks to balle */
 ...
 */
int rfcomm_scan(char *dstAddr, char *srcAddr,  unsigned short int s, unsigned short int e, int o, int d, int t)
{
	...
	for (i = s; i <= e; i++) {
		...
		if (sock >= 0) {
			...
			if (res != 0) {
				...
				if (alrm_flag == 1) {
						printf("rfcomm: %0.2d probably open\n", i);
				}
			}
			else {
...
void usage()
{
	printf("\nrfcomm_scan v0.1.1 (c) Collin R. Mulliner <collin@betaversion.net>\nusage:\n\trfcomm_scan [-i <hci_dev_number>] [-o] "\
			"[-s <channel>] [-e <channel>] <bd_addr>\n\noptions:\n"\
			"\ti\t: number of local hci device\n"\
			"\tt\t: timeout in seconds, default is 3sec\n"\
			"\to\t: only show open channels\n"\
			"\ts\t: start channel (1-30)\n\te\t: end channel (1-30)\n"\
			"\td\t: don't disconnect ACL after each rfcomm connection\n\n"\
			"Note:\n"\
			"\tMake sure the host running rfcomm_scan is configured as YOU want,\n\tif authentication"\
			"and/or encryption is on/off active pairings will be\n\tused or new pairings will start!\n\n ^_^\n\n");
}
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv,"i:e:s:t:od")) != EOF) {
	
		switch(opt) {
		case 'i':
			if(hci_devinfo(atoi(optarg), &di) < 0) 
			{
				perror("HCI device info failed");
				exit(1);
			}
			ba2str(&di.bdaddr, srcAddr);
			break;
		case 'o':
			o = 1;
			break;
	...
	exit(rfcomm_scan(argv[optind], srcAddr, startc, endc, o, d, t));
}
